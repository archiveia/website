	...
	$.fn.pdfEmbedder = function() {
		this.each(function(index, rawAContainer) {
			...
			var callback = function(pdf, showIsSecure) {
				...
				if (pdf === null) {
					divContainer.empty().append($('<div></div>', { class: 'pdfemb-errormsg' }).append((msgnode = $('<span></span>').append(document.createTextNode('Failed to load and decrypt PDF')))));
					return;
				}
				...
				PDFJS.getDocument(pdf).then(
					...
					function(e) {
						...
						if (e.name == 'UnknownErrorException' && e.message == 'Failed to fetch') {
							...
							msgnode = $('<span></span>')
								.append(document.createTextNode(e.message + ' ' + pdfemb_trans.objectL10n.domainerror + ' '))
								.append($('<a href="https://wp-pdf.com/kb/error-url-to-the-pdf-file-must-be-on-exactly-the-same-domain-as-the-current-web-page/" target="_blank">' + pdfemb_trans.objectL10n.clickhereinfo + '</a>'));
						}
